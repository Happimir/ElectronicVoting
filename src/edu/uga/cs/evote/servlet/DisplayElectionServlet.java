...
/**
 * Servlet implementation class DisplayElectionServlet
 */
@WebServlet("/DisplayElectionServlet")
public class DisplayElectionServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DisplayElectionServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {			
			...
			allElections = pLayer.restoreElection(null);
			
			writer.println("var select = document.getElementById('chooseElection');");
			for(int i = 0; i < allElections.size(); i++){
			writer.println("var option = document.createElement('option');");
			writer.println("option.text = \"" + allElections.get(i).getOffice()+"\";");
