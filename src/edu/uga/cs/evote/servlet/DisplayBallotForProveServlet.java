...
/**
 * Servlet implementation class DisplayBallotForProveServlet
 */
@WebServlet("/DisplayBallotForProveServlet")
public class DisplayBallotForProveServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public DisplayBallotForProveServlet() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {			
			...
			writer.println("var select = document.getElementById('chooseBallotForProve');");
			for(int i = 0; i < allBallots.size(); i++){
				writer.println("var option = document.createElement('option');");
				if( !allBallots.get(i).getOpen() && !allBallots.get(i).getApproved())
