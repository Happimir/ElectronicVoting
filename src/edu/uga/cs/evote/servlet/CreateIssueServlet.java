...
public class CreateIssueServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		issue_name = request.getParameter("issuename");
		int ballotID = Integer.parseInt(request.getParameter("chooseBallotForIssue").substring(7));
		if(issue_name == null) {
			writer.println("<script type=text/javascript>");
			writer.println("alert('Issue Name Cannot Be Null');");
			writer.println("</script>");
        	return;
		}
		
		try {
			issue_id = (int) logicLayer.createIssue(issue_name, ballotID);
			writer.println("<script type=text/javascript>");
			writer.println("alert('Issue has been created successfully!');");
